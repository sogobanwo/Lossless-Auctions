...
contract Auction is AutomationCompatibleInterface {
    ...
    function goLive(uint _auctionId) external onlyRegistered {
        AuctionLibrary.goLive(_auctionId, layout);
    }

    function endLive(uint _auctionId) external {
        AuctionLibrary.endLive(_auctionId, layout);
    }
    ...
    function getAllLive() external view returns (AuctionLibrary.AuctionDetails[] memory){
        return AuctionLibrary.getAllLive(layout);
    }
