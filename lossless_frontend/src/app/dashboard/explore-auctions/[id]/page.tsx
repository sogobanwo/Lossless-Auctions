    ...
    <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
      <div className="grid gap-6 lg:grid-cols-[1fr_400px]">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      {...register("bid")}
                      min={
                        auctionDetails?.minValidBid
                          ? Number(
                              ethers.formatEther(auctionDetails.minValidBid)
                            )
                          : 0
                      }
        ...
        <div className="grid gap-6 bg-[#030E19]/70 p-4">
          <div>
            <h1 className="text-3xl font-bold text-[#C9E4CA]">
              {auctionDetails?.name}
            </h1>
            ...
            {auctionDetails?.ended &&
              address === auctionDetails?.higtestBidder && (
                <div>
                  <Button onClick={() => claimWinner()}>Mint Winner NFT</Button>
                </div>
              )}
            {auctionDetails?.ended &&
              address !== auctionDetails?.higtestBidder &&
              auctionParticipants?.includes(address) && (
                <div>
                  <Button onClick={() => participantNFT()}>
          ...
          <div className="grid gap-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-bold text-[#C9E4CA]">
          ...
          <div className="grid gap-4">
            <div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="text-[#C9E4CA]">
                      Participants
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  <TableRow>
                    {auctionParticipants?.map(
                      (participant: any, index: number) => {
                        return (
                          <TableCell key={index} className="text-[#C9E4CA]">
                            {participant}
                          </TableCell>
                        );
                      }
            ...
            <Button
              ...
              onClick={()=>handleJoinLive(params.id)}
            >
              Join Live
            </Button>
          </div>
